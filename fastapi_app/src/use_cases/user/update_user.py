...
class UpdateUserUseCase:
    ...
    def execute(self, user_id: int, user_data: UserUpdate) -> Optional[User]:
        ...
        updated_user = User(
            ...
            email=(
                user_data.email if user_data.email is not None else existing_user.email
            ),
            first_name=(
            ...
            is_active=(
                ...
                else existing_user.is_active
            ),
            is_superuser=(
                user_data.is_superuser
                if user_data.is_superuser is not None
                else existing_user.is_superuser
            ),
            is_staff=(
                user_data.is_staff
                if user_data.is_staff is not None
                else existing_user.is_staff
            ),
