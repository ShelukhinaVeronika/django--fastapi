...
class UpdatePostUseCase:
    ...
    def execute(self, post_id: int, post_data: PostUpdate) -> Optional[Post]:
        ...
        updated_post = Post(
            id=post_id,
            title=(
                post_data.title if post_data.title is not None else existing_post.title
            ),
            text=post_data.text if post_data.text is not None else existing_post.text,
            pub_date=(
                post_data.pub_date
                if post_data.pub_date is not None
                else existing_post.pub_date
            ),
            author_id=existing_post.author_id,
            location_id=(
                post_data.location_id
                if post_data.location_id is not None
                else existing_post.location_id
            ),
            category_id=(
                post_data.category_id
                if post_data.category_id is not None
                else existing_post.category_id
            ),
            image=(
                post_data.image if post_data.image is not None else existing_post.image
            ),
            is_published=(
                post_data.is_published
                if post_data.is_published is not None
                else existing_post.is_published
            ),
