...
class BaseRepository(ABC, Generic[T]):
    ...
    def get_by_id(self, entity_id: int) -> Optional[T]:
        ...
        with self._get_connection() as conn:
            columns_str = ", ".join(self._get_columns())
            query = (
                f"SELECT id, {columns_str} FROM {self._get_table_name()} WHERE id = ?"
            )
    ...
    def update(self, entity_id: int, entity: T) -> Optional[T]:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute(
                f"UPDATE {self._get_table_name()} SET {set_clause} WHERE id = ?", values
            )
            conn.commit()
    ...
    def delete(self, entity_id: int) -> bool:
        ...
        with self._get_connection() as conn:
            cursor = conn.execute(
                f"DELETE FROM {self._get_table_name()} WHERE id = ?", (entity_id,)
            )
            conn.commit()
